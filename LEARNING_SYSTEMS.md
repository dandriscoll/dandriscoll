...
- **Point to depth rather than inlining it.** Some feedback exposes a blind spot whose handling needs detailed, area-specific knowledge that no compact insight can carry. The insight then does two jobs: it names the hazard and when it applies, and it points to a document holding the detail. The agent **SHOULD** ensure that document exists; writing it is part of addressing the feedback. Such a document is operational documentation, not a reference in the §2.2 sense — the evidence trail is kept separately as usual.
